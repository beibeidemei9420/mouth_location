...
def check_zed_sdk_cuda_version(file_path):
    ...
    # result may contains comment
    b= ""
    for b in temp:
        results=re.findall("\d", b)# look for field containing digits
        if(results):
            break
    # split with the . to separate Major and Minor and do not take the last character ')'
    x = re.split("\.", b[:-1])
    ...
    #print("ZED SDK CUDA REQUESTED VERSION: "+str(CUDA_MAJOR_ZED)+"."+str(CUDA_MINOR_ZED))
...
if "aarch64" not in ARCH_VERSION:
